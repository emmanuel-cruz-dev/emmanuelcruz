    ...
    <header className={`flex justify-between items-center p-4 h-20 md:sticky md:top-0 ${
      ...
      <div className='flex gap-4 items-center z-10'>
        <button onClick={toggleTheme} aria-label='Toggle theme' className="flex items-center">
          {theme === 'dark' ? <span title={t('navbar.darkMode')} className="material-icons-outlined">dark_mode</span> : <span title={t('navbar.lightMode')} className="material-icons-outlined">light_mode</span>}
        </button>
        <LanguageSwitcher/>
      <label className='nav__label flex items-center justify-center md:hidden' htmlFor="" aria-label='Toggle menu'>
        <input className='absolute opacity-0 w-full h-full cursor-pointer' type="checkbox" onClick={toggleMenu}/>
        {isMenuOpen ? (
